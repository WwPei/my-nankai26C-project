...
constexpr int kSpawnWaitMs = 1400;
...
QPoint toViewportPoint(QGraphicsView *view, const QPointF &scenePoint)
{
...
private slots:
    ...
    void mouseAimAndLeftClickCreateBullets()
    {
        ...
        const QPointF aimTarget(180.0, 0.0);
        const QPoint viewportPoint = toViewportPoint(view, aimTarget);
        QTest::mouseMove(viewport, viewportPoint);
        QCOMPARE(page->m_mouseScenePosition.toPoint(), aimTarget.toPoint());
    ...
    void enemiesSpawnOverTimeAndStayWithinCap()
    {
        ...
        QTest::qWait(kSpawnWaitMs);

        QVERIFY2(page->m_enemies.size() > 3,
                 "等待刷怪周期后敌人数没有增长。");
